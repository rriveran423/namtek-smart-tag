...
import { FinderHandoff } from "@/components/finder-handoff";
import {
  FinderLanguageProvider,
  FinderLanguageSelector,
  FinderText,
} from "@/components/finder-language";
...
function parseAirport(value: string | null): AirportDetails {
  if (!value)
    return { city: "Not specified", name: "", code: "—", country: "" };
...
function Airport({
  ...
  return (
    <div className="min-w-0 flex-1">
      <div
        className={`flex items-center gap-3 ${direction === "TO" ? "sm:flex-row-reverse" : ""}`}
      >
        {airport.code && (
        ...
        <div className={`min-w-0 ${direction === "TO" ? "sm:text-right" : ""}`}>
          <p className="text-[11px] font-bold tracking-[.14em] text-black/40">
            <FinderText id={direction === "FROM" ? "from" : "to"} />
          </p>
      ...
      {(airport.name || airport.country) && (
        <p
          className={`mt-3 text-sm leading-5 text-black/55 ${direction === "TO" ? "sm:text-right" : ""}`}
        >
          {airport.name}
          {airport.name && airport.country ? " · " : ""}
          {airport.country}
...
export async function generateMetadata({
  ...
  return {
    title: tag
      ? `Found luggage · ${tag.traveler_name ?? "NamTek traveler"}`
      : "Travel tag not found",
        ...
        <div className="mx-auto max-w-[600px]">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3 px-2">
            ...
            <div className="flex items-center gap-2">
              <span className="hidden items-center gap-1.5 text-xs font-bold text-black/45 sm:flex">
                <ShieldCheck size={14} /> <FinderText id="verified" />
              </span>
              <FinderLanguageSelector />
            </div>
          </div>
          <article className="card-shadow overflow-hidden rounded-[34px] border border-black/10 bg-white">
            <header
              className={`relative overflow-hidden px-6 py-9 text-white sm:px-9 ${tag.status === "lost" ? "bg-[#ff5a36]" : "bg-[#2463eb]"}`}
            >
              {tag.traveler_photo_url && (
                <img
                  src={tag.traveler_photo_url}
                  alt={tag.traveler_name ?? "Traveler"}
                  className="mb-5 h-24 w-24 rounded-full border-4 border-white/80 object-cover"
                />
              )}
              <p className="text-xs font-bold uppercase tracking-[.18em] text-white/65">
                NamTek Smart Tag · {tag.public_code}
              </p>
              <h1 className="display mt-4 text-4xl font-extrabold leading-tight">
                <FinderText id="found" values={{ name: firstName }} />
              </h1>
            ...
            <div className="space-y-5 p-6 sm:p-9">
              {tag.bag_photo_url && (
                <section className="overflow-hidden rounded-3xl border border-black/10">
                  <img
                    ...
                    alt={`${tag.luggage_color ?? ""} ${tag.luggage_type ?? "luggage"}`}
                    className="aspect-[4/3] w-full object-cover"
                  />
                  <div className="p-5">
                    <p className="font-bold">
                      {tag.nickname || tag.luggage_type || "Traveler’s luggage"}
                    </p>
                    <p className="mt-1 text-sm text-black/50">
                      {[tag.luggage_color, tag.luggage_brand, tag.luggage_type]
                        .filter(Boolean)
                        .join(" · ")}
                    </p>
                    {tag.luggage_notes && (
              ...
              {(tag.airline || tag.route_origin || tag.route_destination) && (
                <section className="rounded-3xl bg-[#eef4ff] p-5 sm:p-6">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <span className="flex items-center gap-2 font-bold text-[#2454a6]">
                      <Plane size={18} />
                      {tag.airline || "Travel route"}
                    </span>
                    <span className="flex items-center gap-1.5 rounded-full bg-white px-3 py-1.5 text-xs font-bold capitalize">
                      <TripIcon size={14} />
                      {tag.trip_type}
                    </span>
                  </div>
                  <div className="mt-6 grid gap-4 sm:grid-cols-[1fr_auto_1fr] sm:items-center">
                    ...
                    <div className="hidden items-center sm:flex">
                      <span className="h-px w-3 bg-[#2463eb]/25" />
                      <Plane
                        className="mx-1 shrink-0 text-[#2463eb]"
                        size={22}
                      />
                      <span className="h-px w-3 bg-[#2463eb]/25" />
                    </div>
                    <div className="flex items-center gap-2 sm:hidden">
                      <span className="h-5 w-px bg-[#2463eb]/30" />
                      <Plane className="rotate-90 text-[#2463eb]" size={18} />
                    </div>
                  ...
                  {tag.route_stops?.length > 0 && (
                    <div className="mt-5 border-t border-[#2463eb]/15 pt-5">
                      <p className="mb-3 flex items-center gap-1.5 text-xs font-bold tracking-[.12em] text-black/40">
                        <MapPin size={13} /> <FinderText id="connections" />
                      </p>
                      <div className="grid gap-2">
                        {tag.route_stops.map((stop, index) => (
                          <Stop key={stop} value={stop} number={index + 1} />
                        ))}
                      </div>
                    </div>
                  )}
              ...
              <p className="flex items-center justify-center gap-2 text-xs text-black/40">
                <Languages size={14} /> Preferred language:{" "}
                {tag.preferred_language}
              </p>
            </div>
          </article>
          <p className="mt-6 text-center text-xs text-black/40">
            <FinderText id="privacy" />
          </p>
        </div>
      </main>
    </FinderLanguageProvider>
